...
void solve()
{
    ll x, y, k;
    cin >> x >> y >> k;
    ll horizontal = ceil((long double)y / (long double)k);
    ll vertical = ceil((long double)x / (long double)k);
    if (y > x)
    {
        vertical = max(vertical, horizontal);
    }
    else
    {
        horizontal = max(horizontal, vertical - 1);
    }
