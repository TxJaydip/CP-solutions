...
/*
    ...
    ____|__________________________________
        ...
        |
 */
ll fun(ll current, ll n, ll k)
{
    // cout << "for: " << current << ", ";
    n = max(current, n);
    k = min(current, k);
    ll sumLtoR = (current * (current + 1ll) / 2ll) - (k * (k - 1ll) / 2ll);
    ll sumRtoL = _abs64(
        (n * (n + 1ll) / 2ll) - (current * (current + 1ll) / 2ll));
    ll result = _abs64(sumLtoR - sumRtoL);
    // cout << "LtoR: " << sumLtoR << ", RtoL: " << sumRtoL << ", Res: " << result << endl;
...
void solve()
{
    ...
    while (true)
    {
        // cout << "left: " << left << ", right: " << right << ", current: " << current << endl;
        ll nxt = fun(current + 1ll, n, k);
        ll pre = fun(current - 1ll, n, k);
        ll curr = fun(current, n, k);
        if (curr <= pre && curr <= nxt)
        {
            ans = curr;
            break;
        }
