...
int leastFreqFind(map<int, int> &mp)
{
    int leastCnt = 9999999;
    for (auto i : mp)
    {
        leastCnt = min(leastCnt, i.second);
    }

    int lleast2 = 99999;
    for (auto i : mp)
    {
        if (i.second == leastCnt)
        {
            if (lleast2 > i.first)
            {
                lleast2 = i.first;
...
void solve(int n, int q, vector<vi> &op)
{
    map<int, int> mp;
    map<int, int> freq;
    ...
    for (int i = 0; i < op.size(); i++)
    {
        ...
        else
        {
            ...
            if (mp.size() > n)
            {
                int leastFreq = leastFreqFind(mp);
                mp[leastFreq] = -1;
            }
        }
    }
