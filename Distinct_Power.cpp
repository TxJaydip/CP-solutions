...
int indexFromElementInVector(vi &v, ll ele)
{
...
vi giveRank(vi &v)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ans[i] = indexFromElementInVector(v2, v[i]) + 1;
    }
    return ans;
}

vi getRanksAfterRemove(vi &v, int ind)
{
    vi v2 = v;
    // v2[ind] = INT_MIN;
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if ((mp[v[i]] + 1 == mp[v[i - 1]]) || (mp[v[i]] - 1 == mp[v[i - 1]]))
        {
            continue;
        }
        else
