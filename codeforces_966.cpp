...
int main()
{
    ...
    while (t--)
    {
        ...
        ll xs, ys, xt, yt;
        cin >> xs >> ys >> xt >> yt;
        // vector<long long unsigned int> dis;
        long long unsigned int mini = LLONG_MAX;
        long long unsigned int x_d = _abs64(xt - xs);
        long long unsigned int y_d = _abs64(yt - ys);
        // cout << "mul: " << x_d * x_d << ", 2: " << y_d * y_d << ", add: " << (x_d * x_d) + (y_d * y_d) << endl;
        long long unsigned int req_dis = (x_d * x_d) + (y_d * y_d);
        for (pair<long long, long long> &i : v)
        {
            x_d = _abs64(xt - i.first);
            y_d = _abs64(yt - i.second);
            mini = min(mini, ((x_d * x_d) + (y_d * y_d)));
            // dis.pb((pow(xt - i.first, 2) + pow(yt - i.second, 2)));
        }
        if (mini <= req_dis)
        {
            no;
